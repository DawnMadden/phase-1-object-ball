...
function teamColors(namedTeam){
  for (let key in gameObject()){
    let teamName = gameObject()[key].teamName;
      if (teamName === namedTeam) {
...
function teamNames(teamName){
  ...
  for (let key in gameObject()){
    ...
    // return gameObject()[key].teamName;
  } 
  return teamNamesArr;
}
teamNames();

function playerNumbers(namedTeam){
  const numbersOfPlayers = [];
  for (let key in gameObject()){
     let teams = gameObject()[key];  
     if (teams.teamName === namedTeam){
      for (let player in teams.players){
      numbersOfPlayers.push(teams.players[player].number)
      }
     }
    } 
